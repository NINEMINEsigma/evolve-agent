...
15. **视觉项（如有）**：本清单绝大部分是结构性检查，不需视觉；但若涉及观感判断（标签是否重叠、动效是否舒服、整体气质对不对），先 `probe_modality_capability` 确认视觉能力；无视觉（`vision_capable=false`）时把成品 HTML/页面直接展示给用户，由用户亲自判断，不自称“目检通过”。
