...
### 2. 正文（中间部分）
- 第三人称叙事描写当前场景，叙事段落使用默认正文色 `#d4c4a8`
- **对话必须使用独立对话块渲染，不与正文混在同一段落内**

  **对话块样式规范：**

  | 角色类型 | 左边框 | 背景 | 名字色 | 正文色 |
  |---------|--------|------|--------|--------|
  | 玩家/主人 | `#5b9aff` | `rgba(91,154,255,0.08)` | `#5b9aff` | `#b8d4ff` |
  | NPC | `#e94560` | `rgba(233,69,96,0.08)` | `#e94560` | `#f5a0b0` |
  | Eve/特殊 | `#c084fc` | `rgba(192,132,252,0.08)` | `#c084fc` | `#d4b0f0` |
  ...
  **HTML 模板：**
  ...
  <div style="margin:12px 0;padding:10px 14px;
    ...
    <span style="font-weight:bold;color:#5b9aff;">名字标签：</span>
    <span style="color:#b8d4ff;">「对话内容」</span>
  </div>
  ```

- 叙事段落后跟第一个对话块时，之间留一个空行距，不要挤在一起
- 多个对话块连续出现时，各自独立上下排列，不合并
