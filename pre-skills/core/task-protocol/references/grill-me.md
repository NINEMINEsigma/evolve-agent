# Grill Me Pro — 增强版质询协议

> **理念**：实现质量的上限取决于意图的清晰度。本协议的存在是为了在写下任何一行代码之前，先把意图明确化。
...
- 当与 RIPER-5 RESEARCH 模式同时运行时，标注格式为 `[MODE: RESEARCH] [PHASE: ...]`。
...
- 当答案可通过检查获得时，**优先探索代码库**。优先使用 `grep` / `search_files` 和有针对性的文件读取，而非宽泛探索。
...
将识别出的风险记录在决策日志中。详细流程参见 [pre-mortem.md](pre-mortem.md)。
...
2. 生成**决策日志产物**（`decision-log.md`），格式遵循 [decision-log.md](decision-log.md)。

3. 提供交接给下游 skill 的选项（概念参见 [downstream-skills.md](downstream-skills.md)）：
...
- [decision-log.md](decision-log.md) — 决策日志格式
- [pre-mortem.md](pre-mortem.md) — 事前失效分析方法
- [downstream-skills.md](downstream-skills.md) — 下游 skill 概念（规划中）
- [triggers.md](triggers.md) — 各 agent 平台的触发关键词
- [../assets/example-decision-log.md](../assets/example-decision-log.md) — 决策日志示例
- [../scripts/decision_tree_visualizer.py](../scripts/decision_tree_visualizer.py) — 决策树可视化工具