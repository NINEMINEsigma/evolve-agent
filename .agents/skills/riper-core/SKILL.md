...
- 任务文件中已写入的内容不得修改、删除或覆盖重写；出现修正或偏差时必须以追加方式记录修订（见「任务文件修订规则」）
...
任务文件是本任务的审计轨迹，采用「只追加、不改写」原则：

- 任何已写入内容（分析结论、提议的解决方案、实施清单、任务进度条目等）一经写入，不得修改、删除或覆盖重写。
- 出现修正、偏差或计划变更时（例如 EXECUTE 发现偏差返回 PLAN、REVIEW 标记偏差、计划审查后调整、用户提出修正），必须以追加方式记录修订：
  - 在「任务进度」末尾追加带时间戳的修订条目，并明确标注为修正/偏差（也可视需要在文件中新增「修订记录」小节集中存放）；
  - 修订条目必须包含：修订对象（指向被修订的部分/条目编号）、原内容摘要、修订后内容、原因、受影响范围（例如实施清单中哪些编号项受影响）；
  - 若计划整体或部分作废，追加新版本的计划并注明替代关系（如"替代清单第 8/9 项"），原有计划原文保留，不得删除。
- 仅以下例外允许原地更新：
  - 「当前执行步骤」指针字段（同步当前所处步骤）；
  - 「任务进度」「修订记录」「最终审查」本身即为追加式小节，按各自格式向后追加。
- 目的：保留完整的决策与偏差历史，使 REVIEW 能够对照原始计划进行验证，防止执行偏差被静默抹除。
