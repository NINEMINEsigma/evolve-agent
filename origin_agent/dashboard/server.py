...
# AgentLoop reference — set by gateway.server before the server starts.
# Used to expose token usage and tool-call stats from the running agent.
_agent_loop: object | None = None


def set_agent_loop(loop: object) -> None:
    """Wire the AgentLoop into the dashboard for stats access."""
    global _agent_loop
    _agent_loop = loop
            ...
            for line in frontmatter.splitlines():
                ...
                if ":" in line:
                    ...
                    if key == "name":
                        ...
                        # Support comma-separated or YAML list
    ...
    try:
        ...
        if idx_path.exists():
            ...
            for sid in sessions[-20:]:  # last 20
                ...
                # Try to extract message count and last modified from the session file
                msg_count = 0
                last_active = None
                if sp.exists():
...
def _read_log(filename: str, lines: int = 200) -> str:
    # Resolve to guard against path traversal
    try:
...
def register_dashboard_routes(app: FastAPI) -> None:
    ...
    # ── stats endpoints (Stage 3: moderate metrics) ────────────────

    @app.get("/api/stats/token-usage")
    async def stats_token_usage():
        """Return per-session cumulative token consumption."""
    ...
    async def stats_tool_calls():
        """Return tool-call statistics aggregated by tool name."""
    ...
    async def stats_session_activity():
        """Return session counts grouped by time period (today / yesterday / this-week)."""
        from gateway.server import sessions as _sessions
        import time
        now = time.time()
        day = 86400
        today = yesterday = this_week = 0
