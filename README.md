...
- `approval_allowlist.py`：只读 / 自动通过工具的允许列表。
- `mcp_tools.py`：将 MCP server 的工具桥接到 `ToolRegistry`。
- `cron_router.py`：Cron 后台任务的路由与生命周期管理。
- `tools/`：核心工具（filesystem、code、shell、frontend、skills、read_image、run_python、ask_question、progress_tools、clipboard_display_tools 等）。
...
- `mutliagenttools/`：多代理 / 子代理工具集（register/run/chat/approval/stop/list_subagent 等），由 `subagent/orchestrator.py` 调度。
...
- `memory/provider.py`：`EasysaveMemoryProvider` 基于 `third/easysave` 持久化到 `workspace/memory/`。
