...
- `--approval_model_path`, `--approval_model_cuda`, `--approval_model_port`
...
放置 `.gguf` 模型文件，可作为 Adventure 审批模型自动加载。配置项 `approval_model_path` 指向该目录下的模型文件名。
...
| `approval_model_path` | `Qwen3.5-0.8B-Q8_0.gguf` | 审批模型文件名 |
